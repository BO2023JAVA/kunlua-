...
local _print = print
local CardView = apply "androidx.cardview.widget.CardView"
local LinearLayout=apply "android.widget.LinearLayout"
local Spannable=apply "android.text.Spannable"
local Toast=apply "android.widget.Toast"
local EditText=apply "androidx.appcompat.widget.AppCompatEditText"
local InputMethodManager=apply "android.view.inputmethod.InputMethodManager"
local Color=apply "android.graphics.Color"
local WindowManager=apply "android.view.WindowManager"
local TextView=apply "androidx.appcompat.widget.AppCompatTextView"
local LuaAdapter=apply "com.androlua.LuaAdapter"
local Gravity=apply "android.view.Gravity"
local FrameLayout=apply "android.widget.FrameLayout"
local Build=apply "android.os.Build"
local Ticker=apply "com.androlua.Ticker"
local System=apply "java.lang.System"
local Context=apply "android.content.Context"
local HapticFeedbackConstants=apply "android.view.HapticFeedbackConstants"
local ListView=apply "android.widget.ListView"
local ColorStateList=apply "android.content.res.ColorStateList"
local ForegroundColorSpan=apply "android.text.style.ForegroundColorSpan"
local PageView=apply "androidx.viewpager.widget.ViewPager"
local Window=apply "android.view.Window"
local SimpleDateFormat=apply "java.text.SimpleDateFormat"
local Configuration=apply "android.content.res.Configuration"
local Dialog=apply "android.app.Dialog"
local ScrollView=apply "android.widget.ScrollView"
local SpannableStringBuilder=apply "android.text.SpannableStringBuilder"
...
local btn=loadlayout({
  ...
  id='root';
  {
    CardView,
    {
      ...
      layout_marginLeft="2dp",--布局左距
      layout_marginRight="4dp",--布局右距      
      gravity='center';
    }
  }
},ids.btn)
...
when type(k)!='string' k=string.format('[%s]',tostring(k))
  ...
  ids.dia.variable_list.onItemLongClick=function(a,b,c,d)
    ids.dia.text_edit.setText(string.format('%s/%s',table.concat(variable_node,'/'),b.text:match('(.+)%s=>')))
    ids.dia.page.setCurrentItem(3,false)
    return true
  end
  ...
  local function setRipple(id,color)
